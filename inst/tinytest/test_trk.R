...
expect_inherits(tr_v1, "streamline")
unlink(f_v1)
...
expect_inherits(tr_sc, "streamline")
expect_true("FA" %in% colnames(tr_sc))
expect_equal(nrow(tr_sc), 3L)
unlink(f_sc)
...
expect_inherits(tr_pr, "streamline")
expect_true("length" %in% colnames(tr_pr))
unlink(f_pr)
...
expect_inherits(tr_vr, "streamline")
# X coordinates should be shifted by 10, Y by 20, Z by 30
expect_equal(tr_vr[1L, "X"], sl_mat[1L, "X"] + 10, tolerance = 1e-4)
expect_equal(tr_vr[1L, "Y"], sl_mat[1L, "Y"] + 20, tolerance = 1e-4)
expect_equal(tr_vr[1L, "Z"], sl_mat[1L, "Z"] + 30, tolerance = 1e-4)
unlink(f_vr)
