read_header <- function(file_path) {
  ...
  if (length(all_lines) < 4L) {
    if (ext == "tck") {
      cli::cli_abort(
        "File {.file {file_path}} is not in TCK format: too few lines."
      )
    } else {
      cli::cli_alert_warning(
        "File {.file {file_path}} is not in TSF format: too few lines. Skipping file..."
      )
      return(header)
    }
  }
  ...
  if (ext == "tck" && header$id != "mrtrix tracks") {
    cli::cli_abort(
      "File {.file {file_path}} is not in TCK format: invalid first line."
    )
  }
  if (ext == "tsf" && header$id != "mrtrix track scalars") {
    cli::cli_alert_warning(
      "File {.file {file_path}} is not in TSF format: invalid first line. Skipping file..."
    )
    return(header)
  }
...
read_tck <- function(file_path) {
  ...
  num_to_read <- (fs - tck$header$derived$data_offset) /
    tck$header$derived$dsize
...
read_tsf <- function(file_path) {
  ...
  fs <- file.size(file_path)
  num_to_read <- (fs - tsf$header$derived$data_offset) /
    tsf$header$derived$dsize
  fh <- file(file_path, "rb")
  on.exit(
    {
      close(fh)
...
read_mrtrix <- function(input_file) {
  ...
  for (tsf_file in tsf_files) {
    ...
    if (
      is.null(tck_data$header$timestamp) || is.null(tsf_data$header$timestamp)
    ) {
      cli::cli_alert_warning(
        "The field {.field timestamp} is missing from either the TCK or the TSF file. Cannot verify file compatibility using time stamps."
      )
    } else {
      if (tck_data$header$timestamp != tsf_data$header$timestamp) {
        cli::cli_alert_info(
          "Skipping file {.file {tsf_file}} because time stamps do not match."
        )
        next
      }
    }
    if (is.null(tck_data$header$count) || is.null(tsf_data$header$count)) {
      cli::cli_alert_warning(
        "The field {.field count} is missing from either the TCK or the TSF file. Cannot verify file compatibility using streamline counts."
      )
    } else {
      if (tck_data$header$count != tsf_data$header$count) {
        cli::cli_alert_info(
          "Skipping file {.file {tsf_file}} because streamline counts do not match."
        )
        next
      }
    }
