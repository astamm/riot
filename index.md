...
returns a `streamline` when the file contains a single tract and a
`bundle` otherwise. The optional `bundle_data` argument lets you attach
a named list of bundle-level metadata (e.g. subject ID, hemisphere, or
affine transform) directly at import time: the list is stored in the
`@bundle_data` slot of the returned `bundle`.
