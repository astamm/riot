...
header file has been modified to ensure LF line endings and `vtkzlib` source
files have been compiled without checking for protoype warnings when using LLVM
clang compiler. Also, `tinyxml2.cpp` has been modified to avoid the use of
`stdout` and `printf` as per *Writing R Extensions* manual recommendations
because `R` has its own input/output mechanism for writing to the console.
